...
hybridsim<-function(Nmarker,Na,Nb,Nf1,Nbxa=Nf1, type="selection",hybrid="F1",Nsel=Nmarker*0.1,S=0,apa=0.5,apb=0.5){
  if (Na==0 |Nb==0)
  ...
  if (any(hybrid == "all" | hybrid=="BxB")){
    ...
    #Submuestreamos al azar un Numero Nbxa de hibridos con seleccion sobre esos marker
    bxbsel<-progeny[sample(nrow(progeny), Nbxa), ]
  ...
  if (S==0) {sel= NA}
  simulation<-c(simulation,list(SelMarkers=sel))
